...
func RookCephResources(instance *openstackv1beta1.Cinder, imagePools []string) []*unstructured.Unstructured {
	...
	backendsByClient := make(map[cephClientKey][]openstackv1beta1.CinderBackendSpec)
	...
	for client, backends := range backendsByClient {
		...
		for _, pool := range imagePools {
			caps = append(caps, fmt.Sprintf("profile rbd-read-only pool=%s", pool))
		}
...
func filterRookCephBackends(allBackends []openstackv1beta1.CinderBackendSpec) []openstackv1beta1.CinderBackendSpec {
	var backends []openstackv1beta1.CinderBackendSpec
