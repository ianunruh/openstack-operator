...
const (
	PendingReason        = "NovaFlavorPending"
	ReconciledReason     = "NovaFlavorReconciled"
	ReconcileErrorReason = "NovaFlavorReconcileError"
	DeleteErrorReason    = "NovaFlavorDeleteError"
)

func NewReporter(instance *openstackv1beta1.NovaFlavor, k8sClient client.Client, recorder record.EventRecorder) *Reporter {
	return &Reporter{
...
func (r *Reporter) Reconciled(ctx context.Context) error {
	return r.reporter.UpdateReadyCondition(ctx, metav1.ConditionTrue, ReconciledReason, "NovaFlavor is reconciled")
}
